...
public final class ParsingTelemetry: ObservableObject {
  ...
  public func recordTaskCompletion(
    ...
    switch result {
    case .success(let feed, let parsingMetrics):
      eventMetrics = [
    ...
    case .failure(let error, let parsingMetrics):
      eventMetrics = [
        "duration": parsingMetrics.duration,
        "bytes_processed": Double(parsingMetrics.bytesProcessed)
      ]
  ...
  private func updateAggregatedMetrics() {
    ...
    let avgParsingTime = completedTasks.isEmpty ? 0 :
      completedTasks.compactMap { $0.metrics["duration"] }.reduce(0, +) / Double(completedTasks.count)
    
    let cacheHits = Double(completedTasks.filter { 
