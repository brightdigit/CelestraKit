...
public final class MemoryPressureMonitor: ObservableObject {
  ...
  @Published public private(set) var memoryUsage = MemoryUsage()
  ...
  private func updateMemoryUsage() async {
    ...
    if newLevel != currentLevel {
      ...
      logger.info(
        "Memory pressure level changed to \(newLevel) "
          + "(usage: \(usagePercent, format: .number.precision(.fractionLength(1)))%)"
      )
    }
  }
